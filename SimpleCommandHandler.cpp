...
CommandStatus SimpleCommandHandler::Select(const ControlRelayOutputBlock& command, uint16_t index)
{
	...
	char cmd[1024];
    if(index <= 185 && index >= 0){
        std::sprintf(cmd, "sudo python3 /home/scasol-rtu/RTU-ScaSol/RTU/servo/servo_dnp3.py %d", index);
        std::system("cd");
        std::system(cmd);
    } else {
        std::system("cd");
        std::system("sudo python3 /home/scasol-rtu/RTU-ScaSol/RTU/relay/LED.py");
    }
