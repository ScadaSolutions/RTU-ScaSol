...
int main(int argc, char* argv[])
{
    ...
    auto channel = manager.AddTCPClient("tcpclient", logLevels, ChannelRetry::Default(), {IPEndpoint("0.0.0.0", 20000)},
                                        "127.0.0.1", PrintingChannelListener::Create());
    ...
    while (true)
    {
        ...
        case ('l'):
        {
            ...
            master->SelectAndOperate(crob, 421, PrintingCommandResultCallback::Get());
            break;
        }
        case ('s'):
        {   
            ...
            std::cin >> angle;
            ControlRelayOutputBlock crob(OperationType::LATCH_ON);
            master->SelectAndOperate(crob, angle, PrintingCommandResultCallback::Get());
            break;
        }
        case ('t'):
        {
